...
// Copyright (c) 2011-2012 CONTCOIN Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #CONTCOINTEST0\r");
            Send(hSocket, "WHO #CONTCOINTEST0\r");
        } else {
            // randomly join #CONTCOIN00-#CONTCOIN99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #CONTCOIN%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #CONTCOIN%02d\r", channel_number).c_str());
        }
