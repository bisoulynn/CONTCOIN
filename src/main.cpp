...
uint256 hashGenesisBlock("0x0ad607006efd57e17deb28ad23dd94759a0d649e56b0bdee2c284f071116d460");
...
const string strMessageMagic = "CONTCOIN Signed Message:\n";
...
{
    // CONTCOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // CONTCOIN: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "cont genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1506047065;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2302951;
       ...
       // 	block.nTime    = 1388880557;
	    ...
	    //assert(block.hashMerkleRoot == uint256("0x47d260ef5831437b33817fc2bca21ac2f070ac4d894669ee22cacd177638574a"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CONTCOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CONTCOINMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CONTCOINMiner(CWallet *pwallet)
{
    printf("CONTCOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("CONTCOIN-miner");
    ...
    try { loop {
        ...
        printf("Running CONTCOINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CONTCOINMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CONTCOINMiner, pwallet));
}
